...
export default Component.extend(ThrottledResize, {
      ...
      next(() => {
        if (!this.isTransitioning()) {
          ...
          if (currentClusterId) {
            ...
            const entry = get(this, 'byCluster').findBy(
              'clusterId',
              currentClusterId
            );
  ...
  projectChoices: computed(
    'scope.allProjects.@each.{id,displayName,relevantState}',
    function() {
      return get(this, 'scope.allProjects').sortBy('displayName', 'id');
    }
  ),
    ...
    function() {
      ...
      get(this, 'projectChoices').forEach((project) => {
        ...
        const width = getMaxWidth(
          textWidth(get(project, 'displayName'), FONT),
          navWidth
        );
      ...
      function getOrAddCluster(cluster) {
        ...
        let width = getMaxWidth(
          textWidth(get(cluster, 'displayName'), FONT),
          navWidth
        );
  ...
  keyUp(e) {
    if (!isEmpty(this.dropdownApi)) {
      ...
      let tabList = [];
      let $target = $(e.target).hasClass('ember-basic-dropdown-trigger')
        ? $(e.target).find('a')
        : e.target;
...
function highlightMatches(needle, haystack) {
  ...
  while (parts.length) {
    ...
    haystack = haystack.replace(
      placeholder,
      `<span class="search-match">${ escapeHtml(token) }</span>`
    );
  }
