...
export default Component.extend(ViewNewEdit, ChildHook, {
  ...
  actions: {
    ...
    close(saved) {
      ...
      if (get(this, 'step') === 2 && this.done && currentRoute !== 'global-admin.clusters.index'){
        this.done();
      } else if (this.close) {
        this.close(saved);
      }
  ...
  kontainerDrivers: computed(
    ...
    function() {
      ...
      let builtIn = kDrivers.filter(
        (d) => d.state === 'active' &&
          (d.builtIn || BUILD_IN_UI.indexOf(d.id) > -1) &&
          !nope.includes(d.id)
      );
      let custom = kDrivers.filter(
    ...
    function() {
      ...
      if (custom.length > 0) {
        custom.forEach((c) => {
          ...
          const driverEngineSchema = get(this, 'globalStore').getById(
            'schema',
      ...
      get(this, 'model.nodeDrivers')
        ...
        .forEach((driver) => {
          ...
          const driverSchema = get(this, 'globalStore').getById(
            'schema',
            configName.toLowerCase()
          );
  ...
  didSave() {
    ...
    return originalCluster
      .waitForCondition('InitialRolesPopulated')
      .then(() => {
  ...
  doneSaving(saved) {
    if (get(this, 'step') === 1 && get(this, 'driverInfo.preSave')) {
      ...
      $('.cluster-launch ul.breadcrumb li').removeClass('active');
      $('.cluster-launch ul.breadcrumb li:last-child').addClass('active');
    } else {
