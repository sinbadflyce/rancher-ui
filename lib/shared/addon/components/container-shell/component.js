...
export default Component.extend(ThrottledResize, {
  ...
  _bootstrap() {
    set(
      ...
      get(this, 'containerName') ||
        get(this, 'instance.containers.firstObject.name')
    );
  },
  ...
  exec() {
    ...
    if (!url) {
      ...
      url = `${ scheme }localhost/k8s/clusters/${ clusterId }/api/v1/namespaces/${ namespaceId }/pods/${ podName }/exec`;
      url += `?container=${ encodeURIComponent(
        containerName
      ) }&stdout=1&stdin=1&stderr=1&tty=1`;
  ...
  connect(url) {
    ...
    socket.onclose = (err = {}) => {
      ...
      try {
        if (
          !get(this, 'userClosed') &&
          get(this, 'instance.containers.length') === 1
        ) {
          term.destroy();
          if (this.dismiss) {
            this.dismiss();
